...
init_file_lock2();
init_logger2();
import { readdir as readdir3, unlink as unlink4, writeFile as writeFile4 } from "node:fs/promises";
    ...
    try {
      ...
      const result = await withFileLock(stateFile, async () => {
        ...
        if (!hasSignalData(state.totals)) {
          if (state.final) {
            await unlink4(stateFile).catch(() => {});
            logger2.debug(`Cleaned up empty final signal state: ${sessionId}`);
          }
          return null;
        }
        const alreadySynced = state.lastSyncedLine === state.lastReadLine;
        if (alreadySynced && !state.final) {
          return null;
        }
        const normalizedId = normalizeSessionId(sessionId);
        const { error } = await supabase.from("chat_sessions").update({ signals: state.totals }).eq("id", normalizedId);
        if (error) {
          logger2.warn(`Failed to sync signals for session ${sessionId}: ${error.message}`);
          return null;
        }
        const unrecognized = state.unrecognizedToolNames ?? [];
        if (unrecognized.length > 0) {
          logger2.debug(`Unrecognized tool names in session ${sessionId}: ${unrecognized.join(", ")}`);
        }
        if (state.final) {
          try {
            await unlink4(stateFile);
            logger2.debug(`Cleaned up final signal state: ${sessionId}`);
          } catch {}
        } else {
          state.lastSyncedLine = state.lastReadLine;
          await writeFile4(stateFile, JSON.stringify(state), "utf-8");
        }
        return "synced";
      });
      if (result === "synced")
      ...
      logger2.warn(`Signal sync error for ${sessionId}:`, error);
    }
  }
  return synced;
}
