...
async function syncSessionSignals(supabase) {
  ...
  for (const { sessionId } of stateFiles) {
    try {
      ...
      const result = await withFileLock(stateFile, async () => {
        ...
        if (!hasSignalData(state.totals)) {
          if (state.final) {
            ...
            logger.debug(`Cleaned up empty final signal state: ${sessionId}`);
          }
          return null;
        }
        ...
        if (error) {
          logger.warn(`Failed to sync signals for session ${sessionId}: ${error.message}`);
          return null;
        }
      ...
      logger.warn(`Signal sync error for ${sessionId}:`, error);
    }
  }
  return synced;
}
