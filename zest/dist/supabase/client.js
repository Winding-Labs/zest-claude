  ...
  try {
    ...
    if (!daemonClient) {
      ...
      const { data } = client.auth.onAuthStateChange(async (event, _session) => {
        ...
        if (event === "SIGNED_OUT") {
          logger.warn("Implicit SIGNED_OUT (failed refresh), tokens preserved by removal guard");
      ...
      daemonClient = client;
      client = null;
      logger.debug("Daemon Supabase singleton client created with storage adapter");
    }
    const {
      data: { session }
    ...
    await destroyDaemonClient();
    logger.error("Failed to get daemon Supabase client", error);
    if (error instanceof Error) {
      captureException(error, SUPABASE_CLIENT_INIT_FAILED, "supabase/client", {
        has_supabase_url: Boolean(SUPABASE_URL),
        has_anon_key: Boolean(SUPABASE_ANON_KEY),
  ...
  try {
    ...
    if (daemonClient) {
      ...
      try {
        ...
        logger.warn(`Error removing daemon Supabase channels: ${error.message}`);
      }
      daemonClient = null;
