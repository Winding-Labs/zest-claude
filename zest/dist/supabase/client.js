    ...
    [
      ...
      (match) => {
        ...
        if (match && match.length >= 3)
          version3 = isUndefined(match[2]) ? match[3] : match[2];
        return [
          "watchOS",
          version3
        ];
...
function buildStandardProperties(version4) {
  return {
...
async function getDaemonClient() {
  ...
  try {
    ...
    if (!daemonClient) {
      if (!daemonStorage) {
        daemonStorage = createSessionStorageAdapter(() => false);
      }
      client = createSupabaseClientInstance({
        autoRefreshToken: false,
        storage: daemonStorage
      });
      const { data } = client.auth.onAuthStateChange(async (event, _session) => {
        logger2.debug(`Daemon auth state change: ${event}`);
        if (event === "SIGNED_OUT") {
          logger2.warn("Implicit SIGNED_OUT (failed refresh), tokens preserved by removal guard");
      ...
      if (!session2) {
        if (typeof daemonAuthUnsubscribe === "function") {
      ...
      daemonClient = client;
      client = null;
      logger2.debug("Daemon Supabase singleton client created with storage adapter");
    }
    const {
      data: { session }
    } = await daemonClient.auth.getSession();
    if (!session) {
      await destroyDaemonClient();
      return null;
    }
...
async function destroyDaemonClient() {
  ...
  try {
    ...
    if (daemonClient) {
      daemonClient.auth.stopAutoRefresh();
      try {
