...
init_fs_utils2();
init_logger2();
import { readdir as readdir3, readFile as readFile3, rm, stat as stat3, unlink as unlink4, writeFile as writeFile3 } from "node:fs/promises";
...
async function shouldSkipDuplicate(hookType, sessionId) {
  ...
  try {
    await ensureDirectory2(DEBOUNCE_DIR);
    return await withFileLock(debounceFile, async () => {
      ...
      try {
        ...
        if (now - info.timestamp < DEBOUNCE_WINDOW_MS) {
          logger2.info(`Skipping duplicate ${hookType} (within ${DEBOUNCE_WINDOW_MS}ms window, PID ${info.pid} was first)`);
          return true;
        }
      } catch {}
      const newInfo = {
    ...
    logger2.debug(`Debounce check failed for ${hookType}:`, error);
    return false;
  }
}
async function registerHookFired(hookType, sessionId) {
  ...
  try {
    ...
    return await withFileLock(debounceFile, async () => {
      ...
      await writeFile3(debounceFile, JSON.stringify(newInfo), "utf-8");
      logger2.debug(`Registered ${hookType} #${count} for session ${sessionId}`);
      return count;
    });
  } catch (error) {
    logger2.debug("Failed to register hook:", error);
    return 1;
  }
}
    ...
    return await withFileLock(debounceFile, async () => {
      try {
        ...
        if (shouldProcess && info.consumedBy !== process.pid) {
          ...
          logger2.debug(`Not ready to process ${hookType} - only ${msSinceLastHook}ms since last hook (need ${DEBOUNCE_TRAILING_MS}ms)`);
        }
...
async function cleanupDebounceFiles() {
  try {
    const entries = await readdir3(DEBOUNCE_DIR).catch(() => []);
    const now = Date.now();
    ...
    if (cleaned > 0) {
      logger2.debug(`Cleaned up ${cleaned} old debounce entries`);
    }
  } catch {}
}
export {
