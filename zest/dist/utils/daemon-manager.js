...
async function getTrackedPids() {
  try {
    ...
    if (!Array.isArray(registry.pids)) {
      logger2.warn("Claude instances file has invalid format, treating as empty");
      return [];
    }
    ...
    logger2.warn("Failed to read Claude instances file, treating as empty:", error);
    return [];
  }
}
async function writeTrackedPids(pids) {
  ...
  try {
    ...
    logger2.error("Failed to write Claude instances file:", error);
    try {
  ...
  try {
    return await withFileLock(CLAUDE_INSTANCES_FILE, async () => {
      ...
      if (alivePids.length !== pids.length) {
        ...
        logger2.debug(`Pruned ${removedCount} dead Claude instance PID(s)`);
      }
      return alivePids;
    });
  } catch (error) {
    logger2.error("Failed to prune dead PIDs:", error);
    return [];
  }
}
...
import { readFile as readFile4, stat as stat3, unlink as unlink5, writeFile as writeFile4 } from "node:fs/promises";
...
function isWmicAvailable() {
  ...
  try {
    ...
    logger2.info("wmic not available on this system, using PowerShell for process queries");
  }
  return wmicProbeResult === "available";
}
...
async function startDaemon() {
  try {
    ...
    logger2.error("Failed to start daemon:", error);
    if (error instanceof Error) {
      captureException(error, DAEMON_START_FAILED, "daemon-manager", {
...
async function killAllDaemons() {
  try {
    ...
    logger2.warn("Error during daemon cleanup:", error);
  }
}
async function restartDaemon() {
  try {
    return await withFileLock(DAEMON_RESTART_LOCK, async () => {
      ...
      if (existingPid) {
        try {
          const pidFileStat = await stat3(DAEMON_PID_FILE);
    ...
    logger2.error("Failed to restart daemon:", error);
    if (error instanceof Error) {
      captureException(error, DAEMON_RESTART_FAILED, "daemon-manager", {
...
async function writePidFile(pid) {
  try {
    ...
    logger2.debug(`Wrote PID ${pid} to daemon.pid`);
  } catch (error) {
    logger2.error("Failed to write PID file:", error);
  }
}
...
function findClaudeCodePid() {
  ...
  if (grandparentPid && grandparentPid > 1) {
    logger2.debug(`Process tree: self=${process.pid}, parent=${process.ppid}, grandparent=${grandparentPid}`);
    return grandparentPid;
  }
  logger2.debug(`Using direct parent PID: ${process.ppid}`);
  return process.ppid;
}
function cancelShutdownTimer() {
  if (shutdownTimerId !== null) {
    ...
    logger2.info("Claude Code reopened; cancelling daemon shutdown timer");
  }
}
function startShutdownTimer() {
  ...
  logger2.info(`All Claude Code instances closed; daemon will shut down in ${DAEMON_INACTIVITY_TIMEOUT_MS / 1000 / 60} minutes unless reopened`);
...
async function checkClaudeInstancesAndManageShutdown() {
  ...
  logger2.debug(`Claude instances check: tracked=${JSON.stringify(trackedPids)}, alive=${JSON.stringify(alivePids)}, dead=${JSON.stringify(deadPids)}, shutdownTimerActive=${shutdownTimerId !== null}`);
  if (anyClaudeRunning) {
    cancelShutdownTimer();
  } else {
    startShutdownTimer();
...
var DAEMON_RESTART_LOCK, wmicProbeResult = "unknown", __filename2, __dirname2, shutdownTimerId = null;
