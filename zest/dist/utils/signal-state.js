...
async function scanSkillsDir(dir, index, pluginPrefix, relativeDir = "", depth = 0) {
  ...
  for (const entry of entries) {
    ...
    const skillFile = join5(dir, entry.name, "SKILL.md");
    ...
    await scanSkillsDir(join5(dir, entry.name), index, pluginPrefix, childRelative, depth + 1);
  }
}
async function scanCommandsDir(dir, index, pluginPrefix, relativeDir = "", depth = 0) {
  ...
  for (const entry of entries) {
    if (isDirEntry(entry) && !entry.isFile()) {
      const childRelative = relativeDir ? `${relativeDir}/${entry.name}` : entry.name;
      await scanCommandsDir(join5(dir, entry.name), index, pluginPrefix, childRelative, depth + 1);
      continue;
    }
    ...
    if (index.has(id))
      continue;
    const content = await safeReadFile(join5(dir, entry.name));
    if (!content)
      continue;
    const fm = parseFrontmatter(content);
    const description = fm.description ?? extractHeadingDescription(content);
...
async function buildSkillIndex(projectDir) {
  const index = new Map;
  if (projectDir) {
    await scanSkillsDir(join5(projectDir, ".claude", "skills"), index);
  }
    ...
    try {
      ...
      if (registry.plugins) {
        for (const [registryKey, installations] of Object.entries(registry.plugins)) {
          ...
          await scanSkillsDir(join5(install.installPath, "skills"), index, pluginName);
          await scanCommandsDir(join5(install.installPath, "commands"), index, pluginName);
        }
      }
    } catch (error) {
      logger2.debug("Failed to parse installed_plugins.json for skill lookup", error);
    }
  }
  return index;
}
async function buildAgentIndex(projectDir) {
  ...
  if (pluginsContent) {
    try {
      ...
      logger2.debug("Failed to parse installed_plugins.json for agent lookup", error);
    }
  }
  return index;
}
async function scanAgentsDir(dir, index, pluginPrefix, relativeDir = "", depth = 0) {
  ...
  for (const entry of entries) {
    if (isDirEntry(entry) && !entry.isFile()) {
      const childRelative = relativeDir ? `${relativeDir}/${entry.name}` : entry.name;
      await scanAgentsDir(join5(dir, entry.name), index, pluginPrefix, childRelative, depth + 1);
      continue;
    }
    if (!isFileEntry(entry, ".md"))
      continue;
...
async function lookupToolMetadata(toolNames, projectDir) {
  ...
  try {
    ...
    logger2.warn("Failed to look up tool metadata:", error);
  }
  return result;
}
...
async function writeStateToFile(stateFile, state) {
  await ensureDirectory2(STATE_DIR);
...
async function updateSessionSignals(conversationFile, sessionId) {
  try {
    ...
    if (!session) {
      logger2.debug("Not authenticated, skipping session signals update");
      return;
    }
    const stateFile = getSignalStatePath(sessionId);
    ...
    if (newNames) {
      ...
      try {
        ...
        logger2.debug("Failed to enrich tool metadata:", error);
      }
      if (newMetadata) {
        await withFileLock(stateFile, async () => {
          const state = await readStateFromFile(stateFile);
    ...
    logger2.warn("Failed to update session signals:", error);
  }
}
  ...
  try {
    ...
    if (pendingMcpNames) {
      ...
      try {
        ...
        logger2.debug("Failed to enrich MCP tool metadata at finalization:", error);
      }
      if (mcpMetadata) {
        await withFileLock(stateFile, async () => {
          const state = await readStateFromFile(stateFile);
