  ...
  try {
    ...
    if (newNames) {
      ...
      try {
        ...
        logger.debug("Failed to enrich tool metadata:", error);
      }
      if (newMetadata) {
        await withFileLock(stateFile, async () => {
          const state = await readStateFromFile(stateFile);
    ...
    logger.warn("Failed to update session signals:", error);
  }
}
  ...
  try {
    ...
    if (pendingMcpNames) {
      ...
      try {
        ...
        logger.debug("Failed to enrich MCP tool metadata at finalization:", error);
      }
      if (mcpMetadata) {
        await withFileLock(stateFile, async () => {
          const state = await readStateFromFile(stateFile);
