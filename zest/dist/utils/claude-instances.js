  ...
  try {
    await withFileLock(CLAUDE_INSTANCES_FILE, async () => {
      ...
      if (!pids.includes(pid)) {
        ...
        logger2.debug(`Registered Claude instance PID: ${pid}`);
      } else {
        logger2.debug(`Claude instance PID ${pid} already registered`);
      }
    });
  } catch (error) {
    logger2.error("Failed to register Claude PID:", error);
  }
}
  ...
  try {
    await withFileLock(CLAUDE_INSTANCES_FILE, async () => {
      ...
      if (filtered.length !== pids.length) {
        ...
        logger2.debug(`Unregistered Claude instance PID: ${pid}`);
      } else {
        logger2.debug(`Claude instance PID ${pid} was not registered`);
      }
    });
  } catch (error) {
    logger2.error("Failed to unregister Claude PID:", error);
  }
}
