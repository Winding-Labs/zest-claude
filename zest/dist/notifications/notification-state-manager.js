// src/notifications/notification-state-manager.ts
...
var NOTIFICATION_STATE_FILE = join(CLAUDE_ZEST_DIR, "notification-state.json");
...
function readNotificationState() {
  try {
    const data = readFileSync(NOTIFICATION_STATE_FILE, "utf-8");
    const parsed = JSON.parse(data);
    return { ...DEFAULT_STATE, ...parsed };
  } catch (error) {
    if (error.code === "ENOENT") {
      logger.debug("Notification state file does not exist, using defaults");
    } else {
      logger.warn("Failed to read notification state file, using defaults", error);
    }
...
async function writeNotificationState(state) {
  try {
    await ensureDirectory(dirname2(NOTIFICATION_STATE_FILE));
    writeFileSync(NOTIFICATION_STATE_FILE, JSON.stringify(state, null, 2), "utf-8");
    logger.debug("Wrote notification state to file");
  } catch (error) {
    logger.error("Failed to write notification state to file", error);
  }
}
function shouldShowStandupNotification(throttleDurationMs) {
  const state = readNotificationState();
  if (!state.standupNotificationLastShownAt) {
    return true;
  }
  const lastShownTime = new Date(state.standupNotificationLastShownAt).getTime();
  const now = Date.now();
  const elapsedMs = now - lastShownTime;
  return elapsedMs >= throttleDurationMs;
}
async function markStandupNotificationShown() {
  const state = readNotificationState();
  state.standupNotificationLastShownAt = new Date().toISOString();
  await writeNotificationState(state);
  logger.debug("Marked standup notification as shown (throttle timer reset)");
}
export {
  writeNotificationState,
  shouldShowStandupNotification,
  readNotificationState,
  markStandupNotificationShown
};

//# debugId=4F2F6E1C231A195F64756E2164756E21
