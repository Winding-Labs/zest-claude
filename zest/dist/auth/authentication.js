...
var chromeRegexNoFnName, chromeRegex, chromeEvalRegex, chromeStackLineParser = (line, platform2) => {
  ...
  if (parts) {
    ...
    return createFrame(platform2, filename, func, parts[3] ? +parts[3] : undefined, parts[4] ? +parts[4] : undefined);
  }
};
...
var geckoREgex, geckoEvalRegex, geckoStackLineParser = (line, platform2) => {
  ...
  if (parts) {
    ...
    return createFrame(platform2, filename, func, parts[4] ? +parts[4] : undefined, parts[5] ? +parts[5] : undefined);
  }
};
...
var winjsRegex, winjsStackLineParser = (line, platform2) => {
...
var opera10Regex, opera10StackLineParser = (line, platform2) => {
  const parts = opera10Regex.exec(line);
  return parts ? createFrame(platform2, parts[2], parts[3] || UNKNOWN_FUNCTION, +parts[1]) : undefined;
}, opera11Regex, opera11StackLineParser = (line, platform2) => {
...
var FILENAME_MATCH, FULL_MATCH, nodeStackLineParser = (line, platform2) => {
...
var DAEMON_RESTART_LOCK, __filename2, __dirname3;
var init_daemon_manager = __esm(() => {
  ...
  __dirname3 = dirname5(__filename2);
});
...
async function shutdownAnalytics() {
  try {
    if (analyticsClient) {
      ...
      logger2.debug("Analytics client shut down successfully");
    }
  } catch (error) {
    logger2.debug("Error shutting down analytics client", error);
  }
}
...
function getClaudeCodeVersion() {
  ...
  try {
    ...
    logger2.debug("Detected Claude Code version", { version: cachedVersion });
  } catch (error) {
    logger2.debug("Could not detect Claude Code version", error);
...
async function trackExtensionInstalled(userId, version2) {
  try {
    ...
    logger2.debug("Extension installed event tracked", { userId });
  } catch (error) {
    logger2.debug("Failed to track extension installed event", error);
  }
}
...
async function startAuthFlow() {
  try {
    logger2.info("Starting device code authentication flow");
    ...
    logger2.info("Device code authentication successful");
    ...
    if (onDemand) {
      try {
        ...
        try {
          ...
          logger2.warn("Workspace selection failed — sync will be blocked until resolved", wsError);
        }
      } finally {
        await onDemand.dispose();
      }
    } else {
      logger2.warn("Could not create Supabase client for post-login operations");
    }
    return session;
  } catch (error) {
    logger2.error("Device code authentication failed", error);
    throw error;
  }
}
async function initiateDeviceCode() {
  try {
    ...
    logger2.debug("Device code initiated successfully", { deviceCode: data.deviceCode });
    return data;
  } catch (error) {
    logger2.error("Failed to initiate device code", error);
    if (error instanceof Error) {
...
`);
  try {
    ...
    logger2.warn("Failed to open browser automatically", error);
  }
}
  ...
  while (attempts < maxAttempts) {
    ...
    try {
      ...
      if (data.status === "authorized" && data.session) {
        ...
        logger2.info("Device authorized successfully");
        const now = Date.now();
        let refreshTokenExpiresAt;
        ...
        if (refreshTokenExpiresAt) {
          logger2.debug(`Refresh token will expire at ${new Date(refreshTokenExpiresAt).toISOString()}`);
        } else {
          logger2.debug("Refresh token does not expire");
        }
        return {
      ...
      logger2.warn("Polling attempt failed", error);
      networkFailures++;
      await new Promise((resolve) => setTimeout(resolve, deviceCodeData.interval * 1000));
      continue;
    }
...
async function updateProfileMetadata(supabase, userId) {
  try {
    ...
    const { wasAlreadyInstalled } = await updateExtensionMetadata(supabase, userId, "claudeCode", version4, { logger: logger2 });
    if (!wasAlreadyInstalled) {
      await trackExtensionInstalled(userId, version4);
    }
    logger2.info("Profile metadata updated with Claude Code plugin info");
  } catch (error) {
    logger2.warn("Failed to update profile metadata (non-blocking)", error);
  }
}
async function selectDefaultWorkspace(supabase) {
  try {
    ...
    if (!session) {
      logger2.warn("No session found when loading workspace info");
      return;
    }
    logger2.info("Loading workspace information");
    const workspaces = await fetchUserWorkspaces(supabase, session.userId);
    if (workspaces.length === 0) {
      logger2.error("No workspace found for user — auth incomplete");
      console.log(`
⚠️  No workspace found`);
      console.log(`   Create one at: ${WEB_APP_URL}/onboarding
`);
      throw new Error("No workspace available for user — create one via onboarding");
    }
    if (session.workspaceId) {
      ...
      if (selectedWorkspace) {
        ...
        logger2.info("Workspace information loaded", {
      ...
      logger2.warn("Selected workspace not found in user's workspaces, falling back to first workspace");
    }
    ...
    logger2.info("Fallback workspace selected", {
      workspace_id: fallbackWorkspace.id,
      workspace_name: fallbackWorkspace.name,
      total_workspaces: workspaces.length
    });
  } catch (error) {
    logger2.error("Failed to load workspace information", error);
    console.log(`
