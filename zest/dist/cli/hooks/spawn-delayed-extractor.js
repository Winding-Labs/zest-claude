...
// src/cli/hooks/spawn-delayed-extractor.ts
...
function spawnDelayedExtractor(sessionId, conversationFile) {
  try {
    ...
    const isCompiledBinary = !execPath.endsWith("node") && !execPath.endsWith("bun") && !execPath.includes("node") && !execPath.includes("bun");
    let child;
    if (isCompiledBinary) {
      child = spawn(execPath, ["hook:delayed-extract", sessionId, conversationFile], {
        detached: true,
        stdio: "ignore",
        env: process.env
      });
      ...
      if (scriptPath.includes("cli.js") || scriptPath.includes("cli.ts")) {
        child = spawn(process.execPath, [scriptPath, "hook:delayed-extract", sessionId, conversationFile], {
          detached: true,
          stdio: "ignore",
          env: process.env
        });
      } else {
        const { dirname: dirname2, join: join3 } = __require("node:path");
        const delayedExtractorPath = join3(dirname2(scriptPath), "delayed-extractor-cli.js");
        child = spawn("node", [delayedExtractorPath, sessionId, conversationFile], {
          detached: true,
          stdio: "ignore",
          env: process.env
        });
