...
async function getAnalyticsClient() {
  ...
  if (!analyticsClient) {
    ...
    try {
      const { loadSessionFile } = await Promise.resolve().then(() => (init_session_manager2(), exports_session_manager));
...
async function main() {
  try {
    logger2.info("\uD83D\uDD0D Debug: Token Refresh");
    logger2.info(`========================
`);
    const session = await loadSessionFile();
    if (!session) {
      logger2.info("❌ Not authenticated - use /zest:login first");
      return;
    }
    const now = Date.now();
    logger2.info("\uD83D\uDCCA Current Session Info:");
    logger2.info(`   User: ${session.email}`);
    logger2.info(`   User ID: ${session.userId}`);
    if (session.workspaceId) {
      logger2.info(`   Workspace: ${session.workspaceName || session.workspaceId}`);
    }
    const accessTokenExpiry = getJwtExpiryMs(session.accessToken);
    logger2.info(`
\uD83D\uDD11 Access Token Status:`);
    if (accessTokenExpiry) {
      const timeUntilExpiration = accessTokenExpiry - now;
      const isExpired = timeUntilExpiration <= 0;
      logger2.info(`   Expires at: ${new Date(accessTokenExpiry).toISOString()}`);
      logger2.info(`   Status: ${isExpired ? "❌ EXPIRED" : "✅ Valid"}`);
      if (isExpired) {
        logger2.info(`   Expired: ${formatDuration(Math.abs(timeUntilExpiration))} ago`);
      } else {
        logger2.info(`   Time remaining: ${formatDuration(timeUntilExpiration)}`);
      }
    } else {
      logger2.info("   Status: ⚠️  Cannot decode expiry from token");
    }
...
\uD83D\uDD04 Refresh Token Status:`);
      logger2.info(`   Expires at: ${refreshExpiresAt.toISOString()}`);
      logger2.info(`   Status: ${isRefreshExpired ? "❌ EXPIRED" : "✅ Valid"}`);
      if (isRefreshExpired) {
        logger2.info(`   Expired: ${formatDuration(Math.abs(timeUntilRefreshExpiration))} ago`);
        logger2.info(`
❌ Cannot refresh: Refresh token has expired`);
        logger2.info("   Please re-authenticate using /zest:login");
        return;
      }
      logger2.info(`   Time remaining: ${formatDuration(timeUntilRefreshExpiration)}`);
    } else {
      logger2.info(`
\uD83D\uDD04 Refresh Token Status:`);
      logger2.info("   ♾️  Never expires");
    }
    const onDemand = await createOnDemandClient();
    if (!onDemand) {
      logger2.error(`
❌ Failed to get Supabase client`);
      logger2.error("   Check configuration and authentication status");
      return;
    }
    try {
      logger2.info(`
\uD83D\uDD04 Attempting token refresh...`);
      logger2.info("   Using Supabase auto-refresh mechanism");
      const { data, error } = await onDemand.client.auth.refreshSession();
      if (error) {
...
✅ Token refresh successful!`);
      const newTokenExpiry = getJwtExpiryMs(data.session.access_token);
      if (newTokenExpiry) {
        const newTimeUntilExpiration = newTokenExpiry - Date.now();
        logger2.info(`
\uD83D\uDCCA New Token Info:`);
        logger2.info(`   New access token expires at: ${new Date(newTokenExpiry).toISOString()}`);
        logger2.info(`   Time until expiration: ${formatDuration(newTimeUntilExpiration)}`);
      }
      if (session.refreshTokenExpiresAt) {
        logger2.info(`   Refresh token expires at: ${new Date(session.refreshTokenExpiresAt).toISOString()}`);
      } else {
        logger2.info("   Refresh token: Never expires");
      }
      ...
      logger2.info(`
\uD83D\uDD0D Token Changes:`);
      logger2.info(`   Access token changed: ${accessTokenChanged ? "✅ Yes" : "⚠️  No"}`);
      logger2.info(`   Refresh token changed: ${refreshTokenChanged ? "✅ Yes" : "⚠️  No"}`);
      if (!accessTokenChanged) {
        logger2.warn(`
⚠️  Warning: Access token did not change after refresh`);
        logger2.warn("   This might indicate an issue with the refresh endpoint");
      }
      logger2.info(`
✅ Session persisted by storage adapter`);
    } catch (error) {
      logger2.error(`
❌ Token refresh failed!`);
      if (error instanceof Error) {
        ...
        if (error.message.includes("fetch")) {
          logger2.error(`
\uD83D\uDD0D Possible causes:`);
          logger2.error("   - Supabase server is not reachable");
          logger2.error("   - Network connectivity issues");
        } else if (error.message.includes("Invalid Refresh Token") || error.message.includes("invalid_grant")) {
          logger2.error(`
\uD83D\uDD0D Possible causes:`);
          logger2.error("   - Refresh token is invalid or revoked");
          logger2.error("   - User needs to re-authenticate");
        } else if (error.message.includes("500")) {
          logger2.error(`
\uD83D\uDD0D Possible causes:`);
          logger2.error("   - Server error on the refresh endpoint");
          logger2.error("   - Database connection issues");
        }
      }
    ...
    logger2.error("Failed to execute refresh-token command", error);
    logger2.error("❌ Command failed: " + (error instanceof Error ? error.message : "Unknown error"));
    process.exit(1);
  }
}
main();
