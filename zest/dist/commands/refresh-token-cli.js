...
`);
    ...
    if (!session) {
      logger.info("❌ Not authenticated - use /zest:login first");
      return;
    }
    const now = Date.now();
...
\uD83D\uDD11 Access Token Status:`);
    if (accessTokenExpiry) {
      ...
      logger.info(`   Expires at: ${new Date(accessTokenExpiry).toISOString()}`);
      ...
      logger.info("   Status: ⚠️  Cannot decode expiry from token");
    }
...
❌ Cannot refresh: Refresh token has expired`);
        logger.info("   Please re-authenticate using /zest:login");
        return;
      }
...
❌ Failed to get Supabase client`);
      logger.error("   Check configuration and authentication status");
      return;
    }
    try {
      logger.info(`
\uD83D\uDD04 Attempting token refresh...`);
      logger.info("   Using Supabase auto-refresh mechanism");
...
✅ Token refresh successful!`);
      ...
      if (newTokenExpiry) {
        ...
        logger.info(`
\uD83D\uDCCA New Token Info:`);
        logger.info(`   New access token expires at: ${new Date(newTokenExpiry).toISOString()}`);
        logger.info(`   Time until expiration: ${formatDuration(newTimeUntilExpiration)}`);
      }
      if (session.refreshTokenExpiresAt) {
        logger.info(`   Refresh token expires at: ${new Date(session.refreshTokenExpiresAt).toISOString()}`);
      } else {
...
❌ Token refresh failed!`);
      if (error instanceof Error) {
        ...
        if (error.message.includes("fetch")) {
          logger.error(`
\uD83D\uDD0D Possible causes:`);
          logger.error("   - Supabase server is not reachable");
