...
// ../../packages/claude-common/src/utils/uuid.ts
var UUID_REGEX = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;
function isUuid(value) {
  return UUID_REGEX.test(value);
}

// ../../packages/claude-common/src/auth/workspace.ts
...
// ../../packages/claude-common/src/supabase/workspace-fetcher.ts
...
async function main() {
  try {
    ...
    const workspaces = await withSupabaseClient((supabase) => fetchUserWorkspaces(supabase, session.userId, {
      logger,
      onError: (error, { endpoint }) => captureException(error, API_WORKSPACE_FETCH_FAILED, "workspace-fetcher", {
        ...buildApiProperties({ endpoint })
      })
    }));
    if (!workspaces) {
      console.log("❌ Failed to fetch workspaces — authentication may have expired");
      console.log("   Run /zest:login to re-authenticate");
      return;
    }
    ...
    if (args.length === 0) {
      await showWorkspaces(workspaces, displaySession);
      return;
    }
    const input = args[0];
...
async function showWorkspaces(workspaces, session) {
